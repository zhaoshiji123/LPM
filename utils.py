...
def TI_kernel(kernel_size = 5, nsig=3):                              # kernel size
...
def get_kernel2(kernlen=15, nsig=3):
    x = np.linspace(-nsig, nsig, kernlen)
    kern1d = st.norm.pdf(x)
    kernel_raw = np.outer(kern1d, kern1d)
    kernel = kernel_raw / kernel_raw.sum()
...
def TI_kernel2(kernel_size = 5, nsig=3):                              # kernel size
    kernel = (np.ones([3, 5, 5], dtype = int) / (float(kernel_size*kernel_size))).astype(np.float32)
    # kernel = get_kernel2(kernel_size, nsig).astype(np.float32)
    # gaussian_kernel = np.stack([kernel, kernel, kernel])   # 3*5*5
    gaussian_kernel = np.expand_dims(kernel, 1)   # 3*1*5*5
    gaussian_kernel = torch.from_numpy(gaussian_kernel).cuda()  # tensor and cuda
    return gaussian_kernel
...
def input_diversity(x, resize_rate=1.1, diversity_prob=0.7):
    ...
    ret = F.interpolate(ret, (299, 299), mode='bilinear', align_corners=False)
    return ret, (pad_top, pad_left, pad_right, pad_bottom)
...
def DI(x, resize_rate=1.15, diversity_prob=0.5):
    ...
    padded = F.pad(rescaled, [pad_left.item(), pad_right.item(), pad_top.item(), pad_bottom.item()], value=0)
    ret = padded if torch.rand(1) < diversity_prob else x
    ret = F.interpolate(ret, (224, 224), mode='bilinear', align_corners=False)
    return ret
